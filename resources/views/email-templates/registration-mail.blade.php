...
<body>
    ...
    <div class="email-container">
        ...
        <div class="content">
            <h2>Dear {{ $user->name }}, Welcome to {{$websiteSetting->name}}!</h2>
            <p>Thank you for signing up! We're thrilled to have you join our community.</p>
            <p><strong>Use the credentials below to access your account:</strong></p>
            <p>Email: <strong>{{ $user->email }}</strong></p>
            <p>Password: <strong>{{ $password }}</strong></p>
            <a href="{{route('login')}}"><button class="btn btn-primary">Login</button></a>
            
            <p>Enjoy seamless booking for theater events, exclusive access to upcoming shows, and a world of unforgettable experiences.</p>
            <p>Get ready for an exciting journey with <strong>{{$websiteSetting->name}}</strong>!</p>
        </div>
